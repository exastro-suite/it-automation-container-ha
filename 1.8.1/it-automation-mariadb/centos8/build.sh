...
EOS
    ...
    if ! test -e "$SQL_DIR/create-db-and-user_for_MySQL.sql" ; then
        ...
        sed -i -e "s/ITA_DB/${EXASTRO_ITA_DB_NAME}/g" $SQL_DIR/create-db-and-user_for_MySQL.sql
        sed -i -e "s/ITA_USER/${EXASTRO_ITA_DB_USERNAME}/g" $SQL_DIR/create-db-and-user_for_MySQL.sql
        sed -i -e "s/ITA_PASSWD/${EXASTRO_ITA_DB_PASSWORD}/g" $SQL_DIR/create-db-and-user_for_MySQL.sql
        mysql -uroot < $SQL_DIR/create-db-and-user_for_MySQL.sql
    fi
rm -f /tmp/create-db-and-user_for_MySQL.sql

# MariaDB (initialize)
    # mariadb-secure-installation
    send_db_root_password="${DB_ROOT_PASSWORD}"
